import { randomWord } from "./words"
import { Grid, Random } from "./utils"
import { Palette } from "./utils"
import { weightedChoice } from "./utils"
import { Properties } from "./utils"
import { pSet } from "./utils"
import p5 from "p5"
import PoissonDiskSampling from "poisson-disk-sampling"
import { PI2 } from "./utils"

import { frameCount } from "."
import { rng } from "."
import { inputSeed } from "."
import { canvas, width, height } from "."
// import { palette } from "."

export let planets: any
export let satellites: any
export let stars: any


import { weightedChoiceIndex } from "./utils"

export function explore(): void {
  const defaultRandom = { random: Math.random };
  const seed = (inputSeed.value || randomWord.call(defaultRandom)).replace(/ /g, "_").replace(/[^\w]/g, "?").toLowerCase()
  inputSeed.value = ""
  // const rng = new Random(seed.toUpperCase())
  if (seed in Template) {
    (Template as any)[seed]()
  } else {
    generate()
  }
}

function generate(): void {
  const size = Math.max(rng.randint(32, 64), rng.randint(32, 64))

  let pInstance; // p5 인스턴스를 저장할 변수 선언
  const palette = new Palette(
    weightedChoice(
      [Properties.Color.Analogous, Properties.Color.Complementary, Properties.Color.SplitComplementary, Properties.Color.Triad, Properties.Color.Cavity, Properties.Color.Earth],
      [15, 10, 6, 4, 1, 6]
    ),
    new p5((p) => {  // p5 인스턴스 생성을 위한 스케치 함수 제공
      p.setup = function() {
        p.createCanvas(400, 400);
      };
      p.draw = function() {
        p.background(200);
      };
      pInstance = p; // 생성된 p5 인스턴스를 저장
    }),
    rng  // Random 인스턴스
  )
    const isCavity = palette.mode === Properties.Color.Cavity
  
    const noiseDist: number[][] = [
      [3, 1, 2, 1, 2, 2], // Analogous, Complementary
      [3, 0, 2, 0, 0, 2], // SplitComplementary, Triad
      [3, 0, 2, 0, 0, 0] // Cavity, Earth
    ]
    const noiseMode = weightedChoice(
      [Properties.Noise.Simplex, Properties.Noise.Ridged, Properties.Noise.DomainWarping,
      Properties.Noise.VStripe, Properties.Noise.HStripe, Properties.Noise.Gradation],
      noiseDist[Math.floor(palette.mode / 2)]
    )
    const isGradation = noiseMode === Properties.Noise.Gradation
  
    planets = []
    planets.push(new Planet({
      diameter: size,
      noiseMode: noiseMode,
      palette: palette.planet.map(color => color ? color.toString() : null),
      weight: isGradation ? [rng.uniform(1, 4), rng.uniform(1, 4), rng.uniform(1, 4)] : undefined,
      backColor: isCavity ? palette.cloud[0].toString() : undefined,
      lapTime: rng.uniform(3, 5),
      p: pInstance  // p5 인스턴스 전달
    }))
  
    if (!isCavity && weightedChoice([true, false], [4, 1])) {
      planets.push(new Planet({ // cloud
        diameter: size + 4,
        noiseMode: weightedChoice([Properties.Noise.Simplex, Properties.Noise.DomainWarping], [3, 1]),
        palette: [palette.cloud[0].toString(), null, palette.cloud[0].toString()],
        weight: [2, 3, 3],
        backColor: palette.cloud[1].toString(),
        lapTime: planets[0].lapTime * rng.uniform(1.5, 2),
        p: pInstance
      }))
    }
  
    satellites = []
    const hasRing = weightedChoice([true, false], [1, 5])
    for (let i = hasRing ? rng.uniform(2, 4) * size : rng.randint(1, 6); i > 0; i--) {
      satellites.push(new Satellite({
        diameter: rng.randint(2, size / 8),
        color: weightedChoice(palette.satellite, [1, 1]).toString(),
        speed: rng.uniform(0.5, 1.5), // [3sec, 9sec)
        a: rng.randint(size * 3 / 4, size),
        b: rng.randint(size / 8, size / 4),
        initAngle: rng.randint(0, 360),
        rotate: hasRing ? 0 : rng.randint(-90, 90)
      }))
    }


    const width = canvas.width;
    const height = canvas.height;
  
    const pdsObj = new PoissonDiskSampling({
      shape: [width, height],
      minDistance: 25,
      maxDistance: 50,
      tries: 20
    }, rng.random.bind(rng))
    stars = pdsObj.fill().map(val => [...val, weightedChoice([...palette.star, null], [3, 6, 2])])
  }

  export class PixelSphere {
    diameter: number
    sphereWidth: number[]
  
    constructor(diameter: number) {
      this.diameter = diameter
      this.sphereWidth = []
      this._setSphereWidth()
    }
  
    get _sphereWidth(): number[] {
      return this.sphereWidth
    }
  
    _setSphereWidth(): void {
        // Reference: https://github.com/nesbox/TIC-80/blob/813a46bd2ac02f7d28fff57200b715719be60712/src/tic.c#L940-L953
        const parity = 1 - this.diameter % 2
        let r = Math.floor(this.diameter / 2) - parity
        let y = -r
        let x = 0
        let d = 2 - 2 * r
        const i = r
      
        do {
          r = d
          if (r > y || d > x) {
            const w = x * 2 + 1 + parity
            this.sphereWidth[y + i] = w
            this.sphereWidth[this.diameter - y - i - 1] = w
            d += ++y * 2 + 1
          }
          if (r <= x) {
            d += ++x * 2 + 1
          }
        } while (y <= 0)
      }
  }

import { NoiseGenerator } from "./utils"
import { init } from "./utils"
import { FaThinkPeaks } from "react-icons/fa"

  export class Planet extends PixelSphere {
    noiseMode: Properties.Noise
    palette: (string | null)[] 
    weight?: number[]
    lapTime: number
    backColor: string | null
    offset: [number, number]
    noise: NoiseGenerator
    grid: Grid<number>
    speed: number
    hasBack: boolean
    p: p5;
  
    constructor(options: {
      diameter: number,
      noiseMode: Properties.Noise,
      palette: (string | null)[],
      weight?: number[],
      lapTime?: number,
      backColor?: string | null,
      p: p5
    }) {
      super(options.diameter)
      this.noiseMode = options.noiseMode
      this.palette = options.palette
      this.weight = options.weight
      this.lapTime = init(options.lapTime, 1)
      this.backColor = init(options.backColor, null)
      this.offset = [width / 2, height / 2]
      this.noise = new NoiseGenerator(rng.random())
      this.grid = new Grid(this.diameter * 2, this.diameter, 0)
      this._setSphereNoise()
      this.speed = this.diameter / 30 / this.lapTime
      this.hasBack = this.backColor !== null
      this.p = options.p
    }

    _convertVec3(x: number, y: number): [number, number, number] {
        const phi = x / this.grid.width * PI2
        const theta = y / this.grid.height * Math.PI
        const nx = Math.sin(theta) * Math.cos(phi) + 1
        const ny = Math.sin(theta) * Math.sin(phi) + 1
        const nz = Math.cos(theta) + 1
        return [nx, ny, nz]
    }

    _setSphereNoise(): void {
        for (let x = 0; x < this.grid.width; x++) {
          for (let y = 0; y < this.grid.height; y++) {
            let off: number, val: number, weight: number[]
            switch (this.noiseMode) {
              case Properties.Noise.Simplex:
                val = this.noise.simplexFbm(...this._convertVec3(x, y))
                weight = [8, 6, 11]
                break
              case Properties.Noise.Ridged:
                val = this.noise.ridgedFbm(...this._convertVec3(x, y))
                weight = [2, 1, 1]
                break
              case Properties.Noise.DomainWarping:
                val = this.noise.domainWarping(...this._convertVec3(x, y))
                weight = [8, 6, 11]
                break
              case Properties.Noise.VStripe:
                off = this.noise.simplexFbm(...this._convertVec3(x, y))
                val = (Math.cos((4 * x / this.grid.width + off) * this.diameter / 32 * PI2) + 1) * 0.5
                weight = [2, 3, 2]
                break
              case Properties.Noise.HStripe:
                off = this.noise.simplexFbm(...this._convertVec3(x, y))
                val = (Math.cos((4 * y / this.grid.height + off) * this.diameter / 32 * PI2) + 1) * 0.5
                weight = [1, 2, 1]
                break
              case Properties.Noise.Gradation:
                off = this.noise.simplexFbm(...this._convertVec3(x, y))
                val = (y + off * 20) / (this.grid.height + 20)
                weight = [2, 1, 2]
                break
            }
            
            this.grid.set(x, y, weightedChoiceIndex(this.palette.length, this.weight || weight, val))
          }
        }
      }

      draw(isBack: boolean): void {
        if (isBack && !this.hasBack) { return }
        for (let y = 0; y < this.diameter; y++) {
          const sw = this._sphereWidth[y];
          for (let x = 0; x < sw; x++) {
            const gx = Math.floor((x / sw + (isBack ? 1 : 0)) * this.diameter - frameCount * this.speed);
            let c = this.palette[this.grid.get(gx, y)];
            if (isBack && c !== null) {
              c = this.backColor;
            }
            // 색상 문자열을 p5 색상 객체로 변환
            const color = c ? this.p.color(c) : null;
            pSet(this.p, (isBack ? -1 : 1) * (x - sw / 2 + 0.5) + this.offset[0], y + this.offset[1] - this.diameter / 2, color);
          }
        }
      }
    }

export class Satellite extends PixelSphere {
    color: string
    speed: number
    a: number
    b: number
    initAngle: number
    offset: [number, number]
    s: number
    c: number
    p: p5;
  
    constructor(options: {
      diameter: number,
      color: string,
      speed?: number,
      a?: number,
      b?: number,
      initAngle?: number,
      rotate?: number
    }) {
      super(options.diameter)
      this.color = options.color
      this.speed = init(options.speed, 1)
      this.a = init(options.a, width / 3)
      this.b = init(options.b, 0)
      this.initAngle = init(options.initAngle, 0)
      const rotate = init(options.rotate, 0) % 360 * Math.PI / 180
      this.offset = [width / 2, height / 2]
  
      this.s = Math.sin(rotate)
      this.c = Math.cos(rotate)
    }
  
    draw(isBack: boolean): void {
      const rad = (-frameCount - this.initAngle) * this.speed % 360 * Math.PI / 180
      if (isBack !== (Math.abs(rad) < Math.PI)) { return }
      const ex = this.a * Math.cos(rad)
      const ey = this.b * Math.sin(rad)
      const px = ex * this.c - ey * this.s
      const py = ex * this.s + ey * this.c
      for (let y = 0; y < this.diameter; y++) {
        const sw = this._sphereWidth[y]
        for (let x = 0; x < sw; x++) {
          // 색상을 p5 색상 객체로 변환하여 pSet 함수에 전달
          const color = this.p.color(this.color);
          pSet(this.p, px + x + this.offset[0] - sw / 2 + 0.5, py + y + this.offset[1] - this.diameter / 2, color);
        }
      }
    }
  }
  import { gunzipSync } from "zlib"

    export class Template {
        static hexToTable(hex: string): number[] {
          const compressed = hex.match(/[0-9a-f]{2}/g)!.map(val => parseInt(val, 16))
          const decompressed = gunzipSync(Buffer.from(compressed))
          return Array.from(decompressed)
        }
      
        static earth(): void {
          const size = 48
      
          const palette = {
            background: pInstance.color("hsb(255,15%,15%)"),
            planet: [
              pInstance.color("hsb(210,65%,85%)"),
              color("hsb(200,70%,85%)"),
              color("hsb(135,80%,90%)"),
              color("hsb(190,20%,100%)")
            ],
            cloud: [
              color("hsb(0,2%,98%)"),
              color("hsb(0,0%,80%)")
            ],
            satellite: color("hsb(45,20%,90%)"),
            star: [
              color("hsb(255,10%,100%)"),
              color("hsb(255,20%,40%)")
            ]
          }
      
          planets = [
            new Planet({ // earth
              diameter: size,
              palette: palette.planet,
              weight: [1, 1, 1, 1],
              lapTime: 3,
            }),
            new Planet({ // cloud
              diameter: size + 4,
              noiseMode: Properties.Noise.DomainWarping,
              palette: [palette.cloud[0], null, palette.cloud[0]],
              weight: [2, 3, 3.5],
              backColor: palette.cloud[1],
              lapTime: 5
            })
          ]
          planets[0].grid.table = this.hexToTable("")
      
          satellites = [
            new Satellite({
              diameter: 8,
              color: palette.satellite,
              speed: 0.8,
              a: 48,
              b: 8,
              initAngle: rng.randint(0, 360),
              rotate: 30
            })
          ]
      
          const pdsObj = new PoissonDiskSampling({
            shape: [width, height],
            minDistance: 25,
            maxDistance: 50,
            tries: 20
          }, rng.random.bind(rng))
          stars = pdsObj.fill().map(val => [...val, weightedChoice([...palette.star, null], [3, 6, 2])])
        }
      
        static torin(): void { this.yurkth() }
      
        static yurkth(): void {
          const size = 48
      
          palette = {
            background: color("hsb(255,15%,15%)"),
            planet: [
              color("#808080"),
              color("#c7b83c"),
              color("#503c18"),
              color("#92cfbb"),
              color("#3a5345")
            ],
            star: [
              color("hsb(255,10%,100%)"),
              color("hsb(255,20%,40%)")
            ]
          }
      
          planets = [new Planet({
            diameter: size,
            palette: palette.planet,
            weight: [1, 1, 1, 1, 1],
            lapTime: 3,
          })]
          const tmp = new Grid(24, 12)
          tmp.table = this.hexToTable("1f8b0800775ab35e00ff958fd10a402108436fe6ff7f733b53df22b8c3411c65daf75fa9c2bc4385c13b73633562452cac8660c9bc65eececc9bce3c49334fd23be7ce6b81e37b81e3fd01dd6fcce5babff0550758fb0d9b20010000")
          for (let y = 0; y < planets[0].grid.height; y++) {
            for (let x = 0; x < planets[0].grid.width; x++) {
              planets[0].grid.set(x, y, tmp.get(Math.floor(x / 4), Math.floor(y / 4)))
            }
          }
      
          satellites = []
      
          const pdsObj = new PoissonDiskSampling({
            shape: [width, height],
            minDistance: 25,
            maxDistance: 50,
            tries: 20
          }, rng.random.bind(rng))
          stars = pdsObj.fill().map(val => [...val, weightedChoice([...palette.star, null], [3, 6, 2])])
        }
      
        static rainbow(): void {
          const size = 48
      
          palette = {
            background: color("hsb(255,15%,15%)"),
            planet: [
              color("hsb(0,70%,90%)"),
              color("hsb(45,70%,90%)"),
              color("hsb(90,70%,90%)"),
              color("hsb(135,70%,90%)"),
              color("hsb(180,70%,90%)"),
              color("hsb(225,70%,90%)"),
              color("hsb(270,70%,90%)"),
              color("hsb(315,70%,90%)"),
            ],
            star: [
              color("hsb(255,10%,100%)"),
              color("hsb(255,20%,40%)")
            ]
          }
      
          planets = [new Planet({
            diameter: size,
            palette: palette.planet,
            weight: [1, 1, 1, 1, 1, 1, 1, 1],
            lapTime: 3,
          })]
          for (let y = 0; y < planets[0].grid.height; y++) {
            for (let x = 0; x < planets[0].grid.width; x++) {
              planets[0].grid.set(x, y, Math.floor(x / 12) % 8)
            }
          }
      
          satellites = []
      
          const pdsObj = new PoissonDiskSampling({
            shape: [width, height],
            minDistance: 25,
            maxDistance: 50,
            tries: 20
          }, rng.random.bind(rng))
          stars = pdsObj.fill().map(val => [...val, weightedChoice([...palette.star, null], [3, 6, 2])])
        }
      
        static pico8(): void {
          const size = 48
      
          palette = {
            background: color(29, 43, 83),
            planet: [
              color(41, 173, 255),
              color(255, 204, 170),
              color(0, 228, 54),
            ],
            cloud: [
              color(255, 241, 232),
              color(194, 195, 199)
            ],
            satellite: [
              color(255, 163, 0),
              color(255, 236, 39)
            ],
            star: [
              color(255, 241, 232),
              color(95, 87, 79)
            ]
          }
      
          planets = [
            new Planet({ // main planet
              diameter: size,
              noiseMode: Properties.Noise.Simplex,
              palette: palette.planet,
              weight: [8, 1, 6],
              lapTime: 3,
            }),
            new Planet({ // cloud
              diameter: size + 4,
              noiseMode: Properties.Noise.DomainWarping,
              palette: [palette.cloud[0], null, palette.cloud[0]],
              weight: [2, 3, 3],
              backColor: palette.cloud[1],
              lapTime: 5
            })
          ]
      
          satellites = []
          for (let i = 0; i < 2; i++) {
            satellites.push(new Satellite({
              diameter: rng.randint(5, size / 8),
              color: palette.satellite[i],
              speed: rng.uniform(0.5, 1.5), // [3sec, 9sec)
              a: rng.randint(size * 3 / 4, size),
              b: rng.randint(size / 8, size / 4),
              initAngle: rng.randint(0, 360),
              rotate: rng.randint(-90, 90)
            }))
          }
      
          const pdsObj = new PoissonDiskSampling({
            shape: [width, height],
            minDistance: 25,
            maxDistance: 50,
            tries: 20
          }, rng.random.bind(rng))
          stars = pdsObj.fill().map(val => [...val, weightedChoice([...palette.star, null], [3, 6, 2])])
        }
      }