...
export function weightedChoice<T>(array: T[], weight: number[], value: number): T {
...
// Palette 클래스와 관련 타입들은 그대로 유지합니다.
// ...
...
class Palette {
  ...
  satellite: p5.Color[];
  star: p5.Color[];
  planet: (p5.Color | null)[];

  constructor(mode: Properties.Color) {
    this.mode = mode;
    ...
    this.planet = this.generatePlanetColors();
  }

  private generatePlanetColors(): (p5.Color | null)[] {
    switch (this.mode) {
  ...
  parseColor(prop: ColorProperty): p5.Color {
    return color(
...
export { Properties, mod, init, pSet, textb, weightedChoiceIndex, weightedChoice, Grid, Random, NoiseGenerator, Palette };